<?php

namespace App\Database\Migrations;

use CodeIgniter\Database\Migration;

class CreatePermissoes extends Migration
{
    public function up()
    {
        $this->forge->addField([
            'ID_PERMISSAO' => [
                'type' => 'BIGINT',
                'unsigned' => true,
                'auto_increment' => true,
                'comment' => 'Identificador único sequencial da permissão (PK)',
            ],
            'UUID' => [
                'type' => 'CHAR',
                'constraint' => 36,
                'comment' => 'Identificador único público universal (UUID4)',
            ],
            'PERFIL_ID' => [
                'type' => 'BIGINT',
                'unsigned' => true,
                'comment' => 'Chave estrangeira para PERF_PERFIS',
            ],
            'FUNCIONALIDADE_ID' => [
                'type' => 'BIGINT',
                'unsigned' => true,
                'comment' => 'Chave estrangeira para MENU_FUNCIONALIDADES',
            ],
            'ACAO_ID' => [
                'type' => 'BIGINT',
                'unsigned' => true,
                'comment' => 'Chave estrangeira para SIST_ACOES',
            ],
            'PERMITIDO' => [
                'type' => 'TINYINT',
                'comment' => '1 = permitido, 0 = negado (negação explícita)',
            ],
            'SITUACAO_ID' => [
                'type' => 'BIGINT',
                'unsigned' => true,
                'comment' => 'Chave estrangeira para SIST_SITUACOES',
            ],
            'CRIADO_EM' => [
                'type' => 'DATETIME',
                'comment' => 'Data e hora de inserção do registro',
            ],
            'ATUALIZADO_EM' => [
                'type' => 'DATETIME',
                'comment' => 'Data e hora da última modificação',
            ],
            'EXCLUIDO_EM' => [
                'type' => 'DATETIME',
                'null' => true,
                'comment' => 'Data de exclusão lógica (Soft Delete)',
            ],
            'CRIADO_POR' => [
                'type' => 'BIGINT',
                'unsigned' => true,
                'null' => true,
                'comment' => 'ID do usuário que criou o registro',
            ],
            'ATUALIZADO_POR' => [
                'type' => 'BIGINT',
                'unsigned' => true,
                'null' => true,
                'comment' => 'ID do último usuário que alterou o registro',
            ],
            'EXCLUIDO_POR' => [
                'type' => 'BIGINT',
                'unsigned' => true,
                'null' => true,
                'comment' => 'ID do usuário que executou a exclusão lógica',
            ],
        ]);

        $this->forge->addKey('ID_PERMISSAO', true);
        $this->forge->addUniqueKey('UUID');
        $this->forge->addUniqueKey(['PERFIL_ID', 'FUNCIONALIDADE_ID', 'ACAO_ID']);
        $this->forge->addForeignKey('PERFIL_ID', 'PERF_PERFIS', 'ID_PERFIL', 'RESTRICT', 'RESTRICT');
        $this->forge->addForeignKey('FUNCIONALIDADE_ID', 'MENU_FUNCIONALIDADES', 'ID_FUNCIONALIDADE', 'RESTRICT', 'RESTRICT');
        $this->forge->addForeignKey('ACAO_ID', 'SIST_ACOES', 'ID_ACAO', 'RESTRICT', 'RESTRICT');
        $this->forge->addForeignKey('SITUACAO_ID', 'SIST_SITUACOES', 'ID_SITUACAO', 'RESTRICT', 'RESTRICT');
        $this->forge->addForeignKey('CRIADO_POR', 'SEGU_USUARIOS', 'ID_USUARIO', 'RESTRICT', 'RESTRICT');
        $this->forge->addForeignKey('ATUALIZADO_POR', 'SEGU_USUARIOS', 'ID_USUARIO', 'RESTRICT', 'RESTRICT');
        $this->forge->addForeignKey('EXCLUIDO_POR', 'SEGU_USUARIOS', 'ID_USUARIO', 'RESTRICT', 'RESTRICT');
        $this->forge->createTable('PERF_PERMISSOES');
    }

    public function down()
    {
        $this->forge->dropTable('PERF_PERMISSOES');
    }
}
